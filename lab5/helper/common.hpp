...
bool VerifyLab5Result(float *a, float *b, int size) {
  constexpr double eps = 1.e-6;  // machine zero
  bool correct = true;

  for (int i = 0; i < size; i++) {
    double abs_err = std::fabs(a[i] - b[i]);
    double dot_length = size;
    double abs_val = std::fabs(a[i]);
...
class DataLoader {
 public:
  ...
  static void loadData(std::vector<unsigned char> &datas) {
    std::istreambuf_iterator<char> it(std::cin);
    std::istreambuf_iterator<char> end;
    std::vector<unsigned char> buffer(it, end);
    datas = std::move(buffer);
  }
  /*
   * load data like this :
   *
  ...
  static void loadBinary(std::vector<char> &datas, const std::string file) {
    std::ifstream input(file);
    if (!input) {
      fprintf(stderr, "Error open %s by %s\n", file.c_str(), strerror(errno));
      std::abort();
...
class Differ {
 ...
 public:
  ...
  void diff_binary() {
    ...
    std::vector<char> golden;
    DataLoader::loadBinary(result, Argv[1]);
    DataLoader::loadBinary(golden, Argv[2]);
    if (result.size() != golden.size()) {
      fprintf(stderr, "Error result.size & golden.size not matched\n");
      exit(EXIT_FAILURE);
    }
